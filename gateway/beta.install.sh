...
# property of WHICH URL was published (release.burrowee.com/gateway/install.sh
...
if [ -n "$PIN" ]; then
    ...
    if [ -z "$TAG" ]; then
        ...
        if [ "$GH_ANSWERED" = 1 ] && [ "$CHANNEL" = beta ]; then
            fail "no public beta release of gateway right now — ask the operator for a private beta invite link, or install the stable release with install.sh"
        fi
        TAG_SOURCE=catalog
        # GitHub unreachable or no releases published. Try the console catalog
        # (public, no auth): GET ${CONSOLE_URL}/api/v1/releases/<channel>/gateway/current.
        ...
        catalog_url="${CONSOLE_URL}/api/v1/releases/${CHANNEL}/gateway/current"
