...
# pinned preflight sha256, the baked pubkey, the v0.2.13.2026.08.27.08ab6040 floor, the
...
MIN_VERSION="v0.2.13.2026.08.27.08ab6040"
...
if [ -n "$PIN" ]; then
    ...
    if [ -z "$TAG" ]; then
        ...
        # (public, no auth): GET ${CONSOLE_URL}/api/v1/releases/gateway/current?channel=<channel>.
        ...
        catalog_url="${CONSOLE_URL}/api/v1/releases/gateway/current?channel=${CHANNEL}"
