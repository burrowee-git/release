...
# (`@DISPATCHER@ gateway service install`, `doctor --fix`). Both are operator verbs
...
LABEL=com.burrowee.@UNIT_DOT@gateway
UNIT=burrowee-@UNIT_DASH@gateway.service
GUARD_LABEL=com.burrowee.@UNIT_DOT@gateway.guard
...
advance_updater() {
    ...
    Darwin)
        ...
        ;;
    Linux)
        "$SYSTEMCTL" enable --now burrowee-@UNIT_DASH@gateway-updater.service 2>/dev/null || true
        "$SYSTEMCTL" restart burrowee-@UNIT_DASH@gateway-updater.service 2>/dev/null || true
        ;;
    esac
...
@STABLE_ONLY_END@
@BETA_ONLY_BEGIN@
        # NOT under the beta root. Both sweeps hunt 0.2-era copies in the
        # per-user tree and in /usr/local/bin — copies that belong to the
        # STABLE install running beside this one. A beta guard that ran them
        # would tidy away the other channel's binaries and report success.
        log "beta root — skipping the stale-bin and exec-root sweeps (they are the stable install's)"
@BETA_ONLY_END@
        advance_updater
        apply_retention
