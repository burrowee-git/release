...
# (`burroweeb gateway service install`, `doctor --fix`). Both are operator verbs
...
LABEL=com.burrowee.beta.gateway
UNIT=burrowee-beta-gateway.service
GUARD_LABEL=com.burrowee.beta.gateway.guard
...
advance_updater() {
    ...
    Darwin)
        ...
        ;;
    Linux)
        "$SYSTEMCTL" enable --now burrowee-beta-gateway-updater.service 2>/dev/null || true
        "$SYSTEMCTL" restart burrowee-beta-gateway-updater.service 2>/dev/null || true
        ;;
    esac
