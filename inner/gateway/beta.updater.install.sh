...
# or deletes $HOME/.burrowee/gateway, /usr/local/burrowee/beta/etc/gateway,
# /usr/local/burrowee/beta/var/gateway (nor a 0.2 host's not-yet-migrated
...
#                      /usr/local/burrowee/beta/bin, the machine-owned tree's bin/) —
...
# load-bearing; it used to chain through BURROWEE_LINK_DIR, gone with the link
# step.
#
# NO KEEP-LIST GOES WITH IT ANY MORE. The rung used to be handed every
...
SYSTEMD_UPDATER_UNIT="$SYSTEMD_UNIT_DIR/burrowee-beta-gateway-updater.service"
LAUNCHD_PLIST_DIR="${LAUNCHD_PLIST_DIR:-/Library/LaunchDaemons}"
LAUNCHD_UPDATER_PLIST="$LAUNCHD_PLIST_DIR/com.burrowee.beta.gateway.updater.plist"
LAUNCHD_UPDATER_LABEL="com.burrowee.beta.gateway.updater"
...
# tears down com.burrowee.beta.gateway, com.burrowee.beta.gateway.updater and
