...
if [ "$(uname -s)" = "Darwin" ]; then
    ...
    elevate "$SYSTEMCTL" enable --now burrowee-gateway-updater
    elevate "$SYSTEMCTL" restart burrowee-gateway-updater
    echo "updater.install.sh: systemd service burrowee-gateway-updater enabled + (re)started"
fi
