...
# who pointed $COMP_HOME at a directory they owned had that directory taken
...
system_tree_levels() {
    ...
    "$1" "$SYS_BIN_DIR" 0755 || return 1
    "$1" "$(dirname "$SYS_CONFIG_ROOT")" 0755 || return 1
    "$1" "$SYS_CONFIG_ROOT" 0755 || return 1
    "$1" "$COMP_HOME" 0700 || return 1
    "$1" "$(dirname "$SYS_DATA_ROOT")" 0755 || return 1
    "$1" "$SYS_DATA_ROOT" 0755 || return 1
    "$1" "$COMP_DATA" 0700 || return 1
    return 0
}
...
ensure_system_tree() {
    assert_roots_not_symlinked || exit 1
    if ! is_root; then
    ...
    system_tree_levels assert_level_safe_to_state || exit 1
    # PASS 2 — create and state, now that every level has been judged.
    system_tree_levels ensure_dir_stated || exit 1
...
assert_system_tree() {
    ...
    for _ast in "$SYSTEM_ROOT" "$SYS_BIN_DIR" "$SYS_CONFIG_ROOT" "$COMP_HOME" "$SYS_DATA_ROOT" "$COMP_DATA"; do
        ...
        if [ "$_ast_rc" = 2 ]; then
            ...
            echo "hint: directory; then re-run the installer." >&2
        elif [ "$_ast_rc" = 3 ]; then
