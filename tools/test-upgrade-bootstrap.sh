...
# run the inner installer — and then `migrations/upgrade.sh <line>` out of the
...
#   2. THE ARGUMENT — it pins the resolved line, a mismatch refuses, a malformed
#      or extra argument is refused BEFORE the network, and the <line> handed to
#      the ladder is derived from the RESOLVED TAG (so the kit-level cross-check
#      compares the release actually installed against the kit it came from, not
...
#   4. A KIT WITH NO LADDER — refuses, naming the component and the version it
#      just installed, rather than succeeding silently.
#
...
COMP=gateway
STAMP="v0.2.0.2026.08.17.4e43c2ed"
TAG="${COMP}/${STAMP}"
LINE="0.2.0"
...
BURROWEE_PUBKEY_FILE="$W/test.pub" BURROWEE_MIN_VERSION="$LINE" \
...
# TWO kits, same stamp, served from two paths: one WITH migrations/upgrade.sh and
# one without. The stubs record what ran, in what order, and with which
# arguments — argc is recorded separately from arg1 so "passed nothing" and
# "passed the empty string" are different observations, which is exactly the
# difference between deriving the line from the resolved tag and deriving it
# from the operator's (absent) argument.
say "FABRICATE: a signed local release, with and without a ladder"
...
# A THIRD kit whose ladder is PRESENT BUT EMPTY. This is not a hypothetical
...
pass "two signed kits under $W/serve"
...
$MIGRATED" "[$SH] upgrade.sh must run the installer first and the ladder second"
    ...
    # no argument at all the ladder still gets 0.2.0. A bootstrap that forwarded
    # the operator's argument would hand the ladder an empty string here.
...
$MIGRATED" "[$SH] the ladder's line must be derived from the resolved tag"
    ...
    # (5h) The argument pins the line: a mismatch with the resolved release
    ...
    want_out "0.3.0" "[$SH] the refusal must name what was asked for"
    ...
    # (5i) Explicit help is stdout + exit 0 (a refusal is stderr + non-zero).
...
# ---- (6) RESOLVER: the argument pins the resolved line ----------------------
...
# tools/test-version-floor.sh takes, and still no network.
...
resolve() {
    # resolve <line> <floor> -> prints the driver's output; exits non-zero
    # exactly where the shipped bootstrap would abort.
    rm -rf "$W/run-tmp"
    mkdir -p "$W/run-tmp"
    STUB_BLOCKS="$W" STUB_FIXTURES="$W/fx" STUB_FLOOR="$2" STUB_LINE="$1" TMP="$W/run-tmp" \
        sh "$W/resolve-run.sh" 2>&1
}

resolved_tag() { printf '%s\n' "$1" | sed -n 's/^RESOLVED=//p'; }

got="$(resolve "" "$LINE")" || die "unpinned resolution failed:\n$got"
[ "$(resolved_tag "$got")" = "gateway/v0.3.0.2026.09.01.aaaaaaaa" ] \
    || die "with no line the resolver answered '$(resolved_tag "$got")', want the newest release gateway/v0.3.0.2026.09.01.aaaaaaaa"
pass "no line: the newest release wins, as install.sh has always done"

got="$(resolve "$LINE" "$LINE")" || die "line-pinned resolution failed:\n$got"
[ "$(resolved_tag "$got")" = "gateway/v0.2.0.2026.08.17.4e43c2ed" ] \
    || die "line $LINE resolved '$(resolved_tag "$got")' — want the NEWEST build ON THAT LINE, gateway/v0.2.0.2026.08.17.4e43c2ed (0.3.0 is newer overall and must not be chosen)"
pass "a line argument pins resolution to that line, newest build first"

if got="$(resolve "0.4.0" "$LINE")"; then
    die "line 0.4.0 does not exist in the fixture and resolved '$(resolved_tag "$got")' anyway"
fi
case "$got" in
*"0.4.0"*) : ;;
*) die "the no-such-line refusal does not name the line asked for; got:\n$got" ;;
esac
case "$got" in
*"both unreachable"*) die "a line with no release reported GitHub as unreachable, which it was not; got:\n$got" ;;
*) : ;;
esac
pass "a line with no release refuses, naming the line — not 'GitHub unreachable'"
