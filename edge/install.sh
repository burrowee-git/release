...
# pinned preflight sha256, the baked pubkey, the v0.2.17.2026.08.27.3f13bacd floor, the
...
MIN_VERSION="v0.2.17.2026.08.27.3f13bacd"
...
if [ -n "$PIN" ]; then
    ...
    if [ -z "$TAG" ]; then
        ...
        # (public, no auth): GET ${CONSOLE_URL}/api/v1/releases/edge/current?channel=<channel>.
        ...
        catalog_url="${CONSOLE_URL}/api/v1/releases/edge/current?channel=${CHANNEL}"
