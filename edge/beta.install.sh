...
# pinned preflight sha256, the baked pubkey, the v0.3.4.beta.2026.09.04.1e3977a3 floor, the
...
#                                install only to the root-owned /usr/local/burrowee/beta/bin (gateway
...
#                                same /usr/local/burrowee/beta/bin misdirects nothing and is honoured.
...
# property of WHICH URL was published (release.burrowee.com/edge/install.sh
...
MIN_VERSION="v0.3.4.beta.2026.09.04.1e3977a3"
...
# gateway and the edge install to /usr/local/burrowee/beta/bin, root-owned, and
...
# they earned or, if it resolves to /usr/local/burrowee/beta/bin anyway, a line saying
...
# install to /usr/local/burrowee/beta/bin and manage a system service; they cannot install any
...
resolve_elevate() {
    ...
    if ! command -v sudo >/dev/null 2>&1; then
        fail "$COMP installs to /usr/local/burrowee/beta/bin and manages a system service, so it needs root — and sudo is not installed on this host. Re-run this installer as root."
    fi
...
if [ -n "$PIN" ]; then
    ...
    if [ -z "$TAG" ]; then
        ...
        if [ "$GH_ANSWERED" = 1 ] && [ "$CHANNEL" = beta ]; then
            fail "no public beta release of edge right now — ask the operator for a private beta invite link, or install the stable release with install.sh"
        fi
        TAG_SOURCE=catalog
        # GitHub unreachable or no releases published. Try the console catalog
        # (public, no auth): GET ${CONSOLE_URL}/api/v1/releases/<channel>/edge/current.
        ...
        catalog_url="${CONSOLE_URL}/api/v1/releases/${CHANNEL}/edge/current"
...
if [ -z "$MINISIGN" ]; then
    case "$OS" in
      ...
      check the network and the BURROWEE_GH_PROXY mirrors, or install it by hand:
...
run_inner() {
    ...
    if [ -n "$ELEVATE" ]; then
        info "$COMP installs to /usr/local/burrowee/beta/bin and manages a system service — elevating with sudo for the install step (the download and its signature check already ran as $(id -un))"
    fi
    if [ -n "$PREFIX" ]; then
...
if [ "$MODE" = upgrade ]; then
    ...
    ok "installed — no migration ladder is forced under the beta root (/usr/local/burrowee/beta)"
    exit 0
