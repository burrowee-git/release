...
#   curl -fsSL --proto '=https' --tlsv1.2 https://release.burrowee.com/agent/upgrade.sh | sh -s -- 0.2.0
#   curl -fsSL --proto '=https' --tlsv1.2 https://release.burrowee.com/agent/updater.install.sh | sh   (edge/gateway only)
#
...
# pinned preflight sha256, the baked pubkey, the v0.2.5.2026.08.27.54cea691 floor, the
...
# agent have a ladder" belief in this repo that nothing keeps in step with the
...
#
# DO NOT EDIT generated copies (agent/install.sh, agent/upgrade.sh, and for
# edge/gateway agent/updater.install.sh) by hand — they are produced from
...
MIN_VERSION="v0.2.5.2026.08.27.54cea691"
