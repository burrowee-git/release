...
# pinned preflight sha256, the baked pubkey, the v0.2.8.2026.08.27.7a56bdc5 floor, the
...
MIN_VERSION="v0.2.8.2026.08.27.7a56bdc5"
...
if [ -n "$PIN" ]; then
    ...
    if [ -z "$TAG" ]; then
        ...
        # (public, no auth): GET ${CONSOLE_URL}/api/v1/releases/cli/current?channel=<channel>.
        ...
        catalog_url="${CONSOLE_URL}/api/v1/releases/cli/current?channel=${CHANNEL}"
