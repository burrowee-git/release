...
// burrowee-<comp>-<os>-<arch>.zip: two component bins, the dispatcher,
...
func samplePayload() map[string][]byte {
	return map[string][]byte{
		"burrowee-cli":         []byte("cli-binary-bytes"),
		"burrowee-cli-updater": []byte("updater-binary-bytes"),
		"burrowee":             []byte("dispatcher-binary-bytes"),
...
func TestComparePayloadsRezippedEnvelopeOK(t *testing.T) {
	...
	writeZip(t, filepath.Join(candDir, "burrowee-cli-linux-amd64.zip"), payload, time.Unix(2000, 0))
...
func TestComparePayloadsBinaryByteMismatch(t *testing.T) {
	...
	ref := samplePayload()
	cand := samplePayload()
	cand["burrowee-cli"] = []byte("DIFFERENT-cli-binary-bytes")

	writeZip(t, filepath.Join(refDir, "burrowee-cli-linux-amd64.zip"), ref, time.Unix(1000, 0))
	writeZip(t, filepath.Join(candDir, "burrowee-cli-linux-amd64.zip"), cand, time.Unix(1000, 0))
	...
	tr := report.Targets[0]
	if !reflect.DeepEqual(tr.BinMismatch, []string{"burrowee-cli"}) {
		t.Errorf("BinMismatch = %v, want [burrowee-cli]", tr.BinMismatch)
	}
	if !tr.InstallShEqual {
...
func TestComparePayloadsFileSetMismatch(t *testing.T) {
	...
	ref := samplePayload()
	cand := samplePayload()
	cand["burrowee-cli-extra-helper"] = []byte("unexpected-extra-file")
...
func TestComparePayloadsMultipleTargetsSortedByName(t *testing.T) {
	...
	for _, name := range []string{"burrowee-cli-linux-amd64.zip", "burrowee-cli-darwin-arm64.zip"} {
	...
	want := []string{"burrowee-cli-darwin-arm64.zip", "burrowee-cli-linux-amd64.zip"}
	var got []string
